...
exports.getMyProducts = exports.getProductById = exports.deleteProduct = exports.updateProduct = exports.createProduct = void 0;
...
exports.createProduct = (0, asyncHandler_1.default)(async (req, res) => {
    ...
    if (user.role !== "seller")
        throw new ApiError_1.ApiError(403, "Only sellers can create products");
    const { title, description, category, brand, price, salePrice, stock, tags, shippingCost, features, colors, warranty, rating, currency // ✅ Add currency here
     } = req.body;
    if (!title || !category || !price || !stock) {
        throw new ApiError_1.ApiError(400, "Title, category, price, and stock are required.");
    }
    const product = await product_model_1.Product.create({
        ...
        images: req.files && Array.isArray(req.files)
            ? req.files.map((file) => (0, productUpload_1.toPublicUrl)(file.path))
            : [],
    });
...
exports.updateProduct = (0, asyncHandler_1.default)(async (req, res) => {
    ...
    if (!user)
        throw new ApiError_1.ApiError(404, "User not found");
    if (user.role !== "seller")
        throw new ApiError_1.ApiError(403, "Only sellers can update products");
    const productId = req.params.id;
    const product = await product_model_1.Product.findById(productId);
    if (!product)
        throw new ApiError_1.ApiError(404, "Product not found");
    // Destructure incoming fields
    const { title, description, category, brand, price, salePrice, stock, tags, shippingCost, features, colors, warranty, rating, currency, // ✅ Added currency
    ...
    if (req.files && Array.isArray(req.files)) {
        const uploadedImages = req.files.map((file) => (0, productUpload_1.toPublicUrl)(file.path));
...
exports.deleteProduct = (0, asyncHandler_1.default)(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, null, "Product and its media deleted successfully"));
});
// -------------------- GET PRODUCT BY ID --------------------
exports.getProductById = (0, asyncHandler_1.default)(async (req, res) => {
    const productId = req.params.id;
    const product = await product_model_1.Product.findById(productId).populate({
        path: "seller",
        select: "fullName sellerInfo email phone avatar",
    });
    if (!product)
        throw new ApiError_1.ApiError(404, "Product not found");
    res.status(200).json(new ApiResponse_1.ApiResponse(200, product, "Product fetched successfully"));
});
// -------------------- GET ALL PRODUCTS FOR LOGGED-IN SELLER --------------------
exports.getMyProducts = (0, asyncHandler_1.default)(async (req, res) => {
    const userId = req.user?._id;
    if (!userId)
        throw new ApiError_1.ApiError(401, "Unauthorized");
    const userRole = req.user?.role;
    if (userRole !== "seller")
        throw new ApiError_1.ApiError(403, "Only sellers can access their products");
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 20;
    const skip = (page - 1) * limit;
    const searchQuery = { seller: userId };
    if (req.query.title)
        searchQuery.title = { $regex: req.query.title, $options: "i" };
    if (req.query.category)
        searchQuery.category = req.query.category;
    const total = await product_model_1.Product.countDocuments(searchQuery);
    const products = await product_model_1.Product.find(searchQuery)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit);
